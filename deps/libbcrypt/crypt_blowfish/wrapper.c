...
char* __crypt(__const char* key, __const char* setting) {
    return _crypt_retval_magic(__crypt_rn(key, setting, &_ufc_foobar, sizeof(_ufc_foobar)), setting,
                               (char*)&_ufc_foobar, sizeof(_ufc_foobar));
}
#else
...
char* crypt_r(const char* key, const char* setting, void* data) {
    return _crypt_retval_magic(crypt_rn(key, setting, data, CRYPT_OUTPUT_SIZE), setting, (char*)data,
                               CRYPT_OUTPUT_SIZE);
}
...
char* crypt(const char* key, const char* setting) {
    ...
    return _crypt_retval_magic(crypt_rn(key, setting, output, sizeof(output)), setting, output, sizeof(output));
}
...
char* __crypt_gensalt_rn(const char* prefix, unsigned long count, const char* input, int size, char* output,
                         int output_size) {
    char* (*use)(const char* _prefix, unsigned long _count, const char* _input, int _size, char* _output,
                 int _output_size);
...
#ifdef TEST
                static const
    char* tests[][3] = {{"$2a$05$CCCCCCCCCCCCCCCCCCCCC.E5YPO9kmyuRGyh0XouQYb4YMJKvyOeW", "U*U"},
...
int main(void) {
    ...
    for (i = 0; tests[i][0]; i++) {
        ...
        for (o_size = -1; o_size <= (int)sizeof(o_buf); o_size++) {
            ...
            if ((ok_n && (!p || strcmp(p, hash))) || (!ok_n && (!errno || p || strcmp(o_buf, x)))) {
        ...
        if ((ok && (!p || strcmp(p, hash))) || (!ok && (!errno || p || strcmp((char*)data, hash)))) {

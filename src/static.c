...
static void send_error_page(Response* res, http_status status) {
    ...
    set_response_header(res, CONTENT_TYPE_HEADER, "text/html");
    res->status = status;
    send_string(res, error_page);
    free(error_page);
}

static inline void append_or_error(Response* res, Arena* arena, cstr* response, const char* str) {
    if (!cstr_append(arena, response, str)) {
        LOG_ERROR("Failed to append to response\n");
        send_error_page(res, StatusInternalServerError);
        return;
    }
}
...
static void serve_directory_listing(Response* res, const char* dirname, const char* base_prefix) {
    ...
    // Create breadcrumbs
    append_or_error(res, arena, html_response, "<div class=\"breadcrumbs\">");
    append_or_error(res, arena, html_response, "<a href=\"/\">Home</a>");
    ...
    if (!path) {
        ...
        send_string(res, "Failed to allocate memory for path");
        arena_destroy(arena);
        return;
    }
    ...
    while (token) {
        ...
        append_or_error(res, arena, html_response, " / <a href=\"");
        append_or_error(res, arena, html_response, breadcrumb_path);
        append_or_error(res, arena, html_response, "\">");
        append_or_error(res, arena, html_response, token);
        append_or_error(res, arena, html_response, "</a>");
        token = strtok(NULL, "/");
    }
    free(path);

    append_or_error(res, arena, html_response, "</div>");
    ...
    if ((dir = opendir(dirname)) != NULL) {
        while ((ent = readdir(dir)) != NULL) {
            if (strcmp(ent->d_name, ".") != 0 && strcmp(ent->d_name, "..") != 0) {
                append_or_error(res, arena, html_response, "<tr><td><a target=\"_blank\" rel=\"noreferer\" href=\"");
                // Add base prefix if we are not using / as static prefix.
                if (strcmp(base_prefix, "/") != 0) {
                    append_or_error(res, arena, html_response, base_prefix);
                }

                append_or_error(res, arena, html_response, "/");
                append_or_error(res, arena, html_response, ent->d_name);
                append_or_error(res, arena, html_response, "\">");
                append_or_error(res, arena, html_response, ent->d_name);
                append_or_error(res, arena, html_response, "</a></td>");
                ...
                if (stat(filepath, &st) == 0) {
                    ...
                    append_or_error(res, arena, html_response, "<td>Unknown</td>");
                }
                append_or_error(res, arena, html_response, "</tr>");
            }
        }
        closedir(dir);
    } else {
        ...
        set_response_header(res, CONTENT_TYPE_HEADER, "text/html");
        res->status = StatusInternalServerError;
        send_string(res, "Unable to open directory");
        arena_destroy(arena);
        return;
    }

    append_or_error(res, arena, html_response, "</table></body></html>");
    set_response_header(res, CONTENT_TYPE_HEADER, "text/html");
    res->status = StatusOK;
    send_string(res, html_response->data);
    arena_destroy(arena);
}
...
void staticFileHandler(context_t* ctx) {
    ...
    if (n < 0 || n >= MAX_PATH_LEN) {
        ...
        set_response_header(ctx->response, CONTENT_TYPE_HEADER, "text/html");
        ctx->response->status = StatusRequestURITooLong;
        send_response(ctx->response, errmsg, strlen(errmsg));
        return;
    }
    ...
    if (is_dir(filepath)) {
        ...
        if (!path_exists(index_file)) {
            if (browse_enabled) {
                ...
                serve_directory_listing(ctx->response, filepath, prefix);
            } else {
                set_response_header(ctx->response, CONTENT_TYPE_HEADER, "text/html");
                ctx->response->status = StatusForbidden;
                send_string(ctx->response, "<h1>Directory listing is disabled</h1>");
            }
            return;
        } else {
    ...
    if (path_exists(filepath)) {
        ...
        set_response_header(ctx->response, CONTENT_TYPE_HEADER, web_ct);
    ...
    send_response(ctx->response, response, strlen(response));
}
